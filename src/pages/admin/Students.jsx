            ...
            >
                ...
                <div className="mobile-two-col" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                    <div style={{ gridColumn: '1/-1' }}>
                        <label style={ls} htmlFor="student-name">
                            Họ và tên *
                        </label>
                        <input
                            id="student-name"
                    ...
                    <div>
                        <label style={ls} htmlFor="student-dob">
                            Ngày sinh *
                        </label>
                        <input
                            id="student-dob"
                            ...
                            style={is}
                            value={form.dob}
                            onChange={e => hc('dob', e.target.value)}
                        />
                    </div>
                    <div>
                        <label style={ls}>Giới tính</label>
                        <select style={is} value={form.gender} onChange={e => hc('gender', e.target.value)}>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={e => hc('enrollDate', e.target.value)}
                        />
                    </div>
                    <div style={{ gridColumn: '1/-1', background: '#FFF7ED', borderRadius: 10, padding: '12px 14px' }}>
                        ...
                        >
                            <div style={{ gridColumn: '1/-1' }}>
                                <label style={ls} htmlFor="student-parent-name">
                                    Tên phụ huynh *
                                </label>
                                <input
                                    ...
                                    onChange={e => hc('parentName', e.target.value)}
                                />
                            </div>
                            <div>
                                <label style={ls} htmlFor="student-parent-phone">
                                    Số điện thoại
                                </label>
                                <input
                                    ...
                                    onChange={e => hc('parentPhone', e.target.value)}
                                />
                            </div>
                            <div>
                                <label style={ls} htmlFor="student-parent-email">
                                    Email
                                </label>
                                <input
                                    ...
                                    onChange={e => hc('parentEmail', e.target.value)}
                                />
                            </div>
                        </div>
                    </div>
    ...
    return rows
        .map(row => {
            ...
            return {
                ...
                name,
                dob: sanitizeText(
                    normalizeImportDate(pickImportValue(row, ['ngaysinh', 'dob', 'birthday', 'dateofbirth'])),
                ),
                ...
                parentPhone: sanitizeText(
                    pickImportValue(row, ['sdt', 'sodienthoai', 'dienthoai', 'sdtphuhuynh', 'phone', 'parentphone']),
                ),
                parentEmail: sanitizeText(pickImportValue(row, ['email', 'emailphuhuynh', 'parentemail'])),
                enrollDate: sanitizeText(
                    normalizeImportDate(pickImportValue(row, ['ngaynhaphoc', 'ngayvaohoc', 'enrolldate', 'joindate'])),
                ),
...
function LegacyStudents() {
    ...
    function deleteStudent(id) {
        const ndb = getDB()
        ndb.students = ndb.students.filter(s => s.id !== id)
        commit()
        setDB({ ...ndb })
        setModal(null)
        setSelected(null)
        ...
        <div className="admin-page-pad" style={{ padding: '28px 36px' }}>
                    ...
                    try {
                        ...
                        imported.forEach(student => {
                            const classMatch = ndb.classes.find(
                                c => normalizeImportKey(c.name) === normalizeImportKey(student.className),
                            )
                            const existingIndex = ndb.students.findIndex(
                                s =>
                                    (student.code &&
                                        normalizeImportKey(s.code || s.studentCode || s.id) ===
                                            normalizeImportKey(student.code)) ||
                            ...
                            const record = {
                                ...
                                id:
                                    existingIndex >= 0
                                        ? ndb.students[existingIndex].id
                                        : student.code || 's' + Date.now() + Math.random(),
                                classId: classMatch?.id || 'c1',
                                enrollDate: student.enrollDate || todayStr(),
                            }
                            if (existingIndex >= 0)
                                ndb.students[existingIndex] = { ...ndb.students[existingIndex], ...record }
                        ...
                        setImportMsg('❌ File không đúng định dạng')
                        setTimeout(() => setImportMsg(''), 3000)
                    }
                    e.target.value = ''
                }}
            ...
            >
                <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    <button
                        onClick={() => exportCSV(filtered, db.classes)}
                        style={{
                            padding: '10px 18px',
                            borderRadius: 12,
                            border: '1.5px solid #DDD6FE',
            ...
            >
                                ...
                                >
                                    ...
                                    <td style={{ padding: '12px 16px' }}>
                                        <span
                                            style={{
                                                fontSize: 12,
                                                fontWeight: 700,
                                                color: cls?.color || '#6B6494',
                                                background: (cls?.color || '#6B6494') + '18',
                                                borderRadius: 6,
                                                padding: '2px 8px',
                                            }}
                                        >
                                    ...
                                    <td style={{ padding: '12px 16px' }}>
                                        <div style={{ display: 'flex', gap: 6 }}>
                                            ...
                                            <button
                                                onClick={() => {
                                                    if (confirm('Xóa học sinh này?')) deleteStudent(s.id)
                                                }}
                                                style={{
                                                    padding: '5px 12px',
                                                    borderRadius: 8,
                ...
                {filtered.length === 0 && (
                    <div style={{ textAlign: 'center', padding: '40px', color: '#7C6D9B', fontSize: 14 }}>
