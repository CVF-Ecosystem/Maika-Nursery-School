...
export default function CalendarView() {
    ...
    function addEvent() {
        ...
        const ndb = getDB()
        ndb.events.unshift({ ...form, id: 'e' + Date.now() })
        commit()
        setDB({ ...ndb })
        setModal(false)
        setForm({ title: '', date: '', type: 'school', desc: '' })
    }
    ...
    function EventRow({ e }) {
        const col = TYPE_COLOR[e.type] || '#6B6494'
        const lbl = TYPE_LABEL[e.type] || e.type
        ...
        return (
            ...
            >
                <div style={{ textAlign: 'center', width: 48, flexShrink: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: '#9B93C9' }}>
                ...
                <span
                    style={{
                        fontSize: 12,
                        color: e.date === today ? '#16A34A' : '#9B93C9',
                        fontWeight: 700,
                        background: e.date === today ? '#F0FDF4' : 'transparent',
                        borderRadius: 6,
                        padding: '2px 8px',
                        flexShrink: 0,
                    }}
                >
                    {e.date === today ? 'Hôm nay' : ''}
                </span>
            </div>
        )
    }
    ...
    return (
        <div className="admin-page-pad" style={{ padding: '28px 36px' }}>
                ...
                >
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                            <div>
                                <label style={ls}>Tên sự kiện *</label>
                                <input
                                    style={is}
                                    value={form.title}
            ...
            >
                ...
                {upcoming.length === 0 ? (
                    <div style={{ padding: '30px', textAlign: 'center', color: '#7C6D9B' }}>
            ...
            {past.length > 0 && (
                <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 16px rgba(109,40,217,0.08)' }}>
                    <div
                        style={{
                            padding: '16px 18px',
                            borderBottom: '1.5px solid #EDE9FE',
