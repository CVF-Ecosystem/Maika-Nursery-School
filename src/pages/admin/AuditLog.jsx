...
function ActionBadge({ action }) {
    ...
    return (
        <span style={{ color, background: bg, borderRadius: 6, padding: '3px 9px', fontWeight: 800, fontSize: 11 }}>
                ...
                >
                    {actions.map(action => (
                        <option key={action} value={action}>
                            {action === 'all' ? 'Tất cả hành động' : ACTION_LABEL[action] || action}
                        </option>
                    ))}
                </select>
                <select
                ...
                >
                    {entityTypes.map(entity => (
                        <option key={entity} value={entity}>
                            {entity === 'all' ? 'Tất cả đối tượng' : entity}
                        </option>
                    ))}
                </select>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                        <tr style={{ background: '#F8F7FF' }}>
                            {['Thời gian', 'Hành động', 'Người dùng', 'Đối tượng', 'Mô tả'].map(h => (
                                <th
                                    key={h}
                                    style={{
                                        padding: '12px 16px',
                    ...
                    <tbody>
                        {logs.map(log => (
                            <tr key={log.id} style={{ borderBottom: '1px solid #EDE9FE' }}>
                                ...
                                <td style={{ padding: '12px 16px', fontSize: 13, color: '#1E1B4B', fontWeight: 700 }}>
                                    {log.summary}
                                </td>
                            </tr>
                        ))}
