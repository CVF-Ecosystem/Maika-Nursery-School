...
function LegacyAttendance() {
    ...
    function markAll(status) {
        ...
        students.forEach(s =>
            ndb.attendance.push({ id: `att-${selDate}-${s.id}`, studentId: s.id, date: selDate, status, note: '' }),
        )
        commit()
        setDB({ ...ndb })
    }
    ...
    const btn = (active, color) => ({
        padding: '5px 12px',
        borderRadius: 8,
        border: `1.5px solid ${active ? color : '#DDD6FE'}`,
        background: active ? color : '#fff',
        color: active ? '#fff' : '#6B6494',
        fontWeight: 700,
        fontSize: 12,
        cursor: 'pointer',
    ...
    return (
        <div className="admin-page-pad" style={{ padding: '28px 36px' }}>
            ...
            >
                {[
                    ['✅', 'Có mặt', present, '#16A34A', '#F0FDF4'],
                    ['⏰', 'Đi trễ', late, '#7C3AED', '#F5F3FF'],
                    ['❌', 'Vắng mặt', absent, '#DC2626', '#FEF2F2'],
                    ['⬜', 'Chưa điểm danh', unmarked, '#6B6494', '#F5F5F4'],
                ].map(([icon, lbl, cnt, col, bg]) => (
                    <div
                        key={lbl}
                        style={{
                            background: bg,
                    ...
                    >
                        ...
                        <div>
                            <div style={{ fontWeight: 900, fontSize: 24, color: col, lineHeight: 1 }}>{cnt}</div>
                            <div style={{ fontSize: 12, color: col, fontWeight: 700 }}>{lbl}</div>
                        </div>
                    </div>
                ))}
            </div>
                                    ...
                                    <td style={{ padding: '10px 16px' }}>
                                        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-start' }}>
                                            <button
                                                ...
                                                style={btn(att?.status === 'present', '#16A34A')}
                                            >
                                                ✅ Có mặt
                                            </button>
                                            <button
                                                onClick={() => setStatus(s.id, 'late')}
                                                style={btn(att?.status === 'late', '#7C3AED')}
                                            >
                                                ⏰ Trễ
                                            </button>
                                            <button
                                                onClick={() => setStatus(s.id, 'absent')}
                                                style={btn(att?.status === 'absent', '#DC2626')}
                                            >
                                                ❌ Vắng
                                            </button>
                                        </div>
