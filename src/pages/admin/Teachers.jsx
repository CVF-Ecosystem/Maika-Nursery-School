            ...
            >
                ...
                <div className="mobile-two-col" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                    <div style={{ gridColumn: '1/-1' }}>
                        <label style={ls}>Họ và tên *</label>
                        <input style={is} value={form.name} onChange={e => hc('name', e.target.value)} />
                    </div>
                    <div>
                        <label style={ls}>Chuyên môn</label>
                        <input style={is} value={form.subject} onChange={e => hc('subject', e.target.value)} />
                    </div>
                    <div>
                        <label style={ls}>Phụ trách lớp</label>
                        {db?.classes?.length ? (
                            <select style={is} value={form.classId || ''} onChange={e => hc('classId', e.target.value)}>
                            ...
                            <input
                                ...
                                onChange={e => hc('className', e.target.value)}
                                placeholder="VD: Lớp Mầm"
                            />
                        )}
                    </div>
                    <div>
                        <label style={ls}>Điện thoại</label>
                        <input style={is} value={form.phone} onChange={e => hc('phone', e.target.value)} />
                    </div>
                    <div>
                        <label style={ls}>Email</label>
                        <input style={is} value={form.email} onChange={e => hc('email', e.target.value)} />
                    </div>
                    <div>
                        <label style={ls}>Ngày vào làm</label>
                        <input
                            type="date"
                            style={is}
                            value={form.joinDate}
                            onChange={e => hc('joinDate', e.target.value)}
                        />
                    </div>
                    <div>
                        <label style={ls}>Trình độ</label>
                        <input style={is} value={form.degree} onChange={e => hc('degree', e.target.value)} />
                    </div>
                    <div>
                        <label style={ls}>Trạng thái</label>
                        <select style={is} value={form.status} onChange={e => hc('status', e.target.value)}>
    ...
    return rows
        .map(row => {
            ...
            return {
                ...
                subject:
                    pickImportValue(row, ['chuyenmon', 'mon', 'vaitro', 'chucvu', 'vitri', 'nhiemvu', 'subject']) ||
                    'Giáo viên chủ nhiệm',
...
function SupabaseTeachers({ selectedFacilityId = '', facilities = [] }) {
    ...
    return (
        <div className="admin-page-pad" style={{ padding: '28px 36px' }}>
            ...
            >
                <div style={{ fontSize: 13, color: '#7C6D9B', fontWeight: 800 }}>
                    {facility ? `${facility.code} - ${facility.name}` : 'Chưa chọn cơ sở'}
                </div>
                <div
                    style={{
                        display: 'flex',
            ...
            {loading ? (
                ...
                >
                    <div style={{ fontWeight: 800, color: '#1E1B4B', marginBottom: 6 }}>
                        Chưa có giáo viên trong cơ sở này
                    </div>
                    <div style={{ fontSize: 13 }}>
                        Bạn có thể thêm thủ công tại đây hoặc import hồ sơ giáo viên sau.
                    </div>
                </div>
            ) : (
                <div
                    ...
                    {teachers.map(teacher => (
                        ...
                        >
                            <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start', marginBottom: 16 }}>
                                <Avatar
                                    initials={teacher.initials || initialsFromName(teacher.name || teacher.email)}
                                    size={50}
                                />
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    ...
                                    <div style={{ fontSize: 13, color: '#7C6D9B', marginTop: 2 }}>
                                        {teacher.subject}
                                    </div>
                                    {teacher.className && (
                                        <span
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                                ...
                                <div style={{ fontSize: 13, color: '#6B6494' }}>
                                    📅 Vào làm: {teacher.joinDate ? fmtDate(teacher.joinDate) : '—'}
                                </div>
                            </div>
...
function LegacyTeachers() {
    ...
    async function handleImport(file) {
        ...
        try {
            ...
            const ndb = getDB()
            rows.forEach(row => {
                const classMatch = ndb.classes.find(
                    c => normalizeImportKey(c.name) === normalizeImportKey(row.className),
                )
                const existingIndex = ndb.teachers.findIndex(
                    t =>
                ...
                const record = {
                    ...
                    id: existingIndex >= 0 ? ndb.teachers[existingIndex].id : 't' + Date.now() + Math.random(),
                }
                ...
                {importMsg && (
                    <span
                        style={{
                            fontSize: 13,
                            fontWeight: 800,
                            color: importMsg.startsWith('Không') ? '#DC2626' : '#059669',
                            background: importMsg.startsWith('Không') ? '#FEF2F2' : '#ECFDF5',
                            borderRadius: 8,
                            padding: '7px 12px',
                        }}
                    >
                    ...
                    return (
                        ...
                        >
                            <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start', marginBottom: 16 }}>
                                ...
                                >
                                    {t.status === 'active' ? 'Đang làm' : 'Đã nghỉ'}
                                </span>
                            </div>
