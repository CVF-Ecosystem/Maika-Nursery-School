...
export function buildFeeNoticeHtml(notice, items, student, settings = getPaymentSettings()) {
...
export default function FeeNoticePrint({ notice, items = [], student, label = 'In phiếu' }) {
    function exportPdf() {
        ...
        const bodyHtml = buildFeeNoticeHtml(notice, items, student, settings)
        const w = window.open('', '_blank', 'width=700,height=1050')
        if (!w) return
        w.document.write(`<!DOCTYPE html><html><head>
<meta charset="utf-8">
<title>Phiếu báo thu ${notice.notice_number || ''}</title>
<style>${NOTICE_PAGE_CSS}</style></head><body>
${bodyHtml}
