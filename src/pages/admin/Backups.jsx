...
import {
    createArchivedMediaDownload,
    deleteArchivedMedia,
    getMediaStorageSummary,
} from '../../features/media/storageMaintenanceService'
...
function SchedulerStatus({ scheduler }) {
    ...
    return (
        <div
            style={{
                background: enabled ? '#F0FDF4' : '#F8F7FF',
                borderRadius: 14,
                padding: '16px 20px',
                marginBottom: 20,
                border: `1.5px solid ${enabled ? '#BBF7D0' : '#DDD6FE'}`,
            }}
        >
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <div
                    style={{ width: 10, height: 10, borderRadius: 999, background: enabled ? '#16A34A' : '#6B6494' }}
                    aria-hidden="true"
                />
            ...
            {enabled && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px,1fr))', gap: 10 }}>
                    <div style={{ background: '#fff', borderRadius: 10, padding: '10px 14px' }}>
                        <div style={{ fontSize: 10, fontWeight: 800, color: '#7C6D9B', marginBottom: 2 }}>
                            LỊCH CRON
                        </div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: '#1E1B4B', fontFamily: 'monospace' }}>
                            {cron}
                        </div>
                    </div>
                    <div style={{ background: '#fff', borderRadius: 10, padding: '10px 14px' }}>
                        ...
                        <div style={{ fontSize: 13, fontWeight: 700, color: '#1E1B4B' }}>
                            {retentionCount} bản · {retentionDays} ngày
                        </div>
                    </div>
                    <div style={{ background: '#fff', borderRadius: 10, padding: '10px 14px' }}>
                        <div style={{ fontSize: 10, fontWeight: 800, color: '#7C6D9B', marginBottom: 2 }}>
                        ...
                        <div
                            style={{
                                fontSize: 13,
                                fontWeight: 700,
                                color: lastResult?.success === false ? '#DC2626' : '#1E1B4B',
                            }}
                        >
                            {lastRun ? fmtDate(lastRun) : 'Chưa chạy'}
                            {lastResult?.success === false && (
                                <span style={{ color: '#DC2626', fontSize: 11, display: 'block' }}>
                                    Lỗi: {lastResult.error}
                                </span>
                            )}
                            {lastResult?.success && (
                                <span style={{ color: '#16A34A', fontSize: 11, display: 'block' }}>
                                    ✓ {lastResult.backup}
                                    {lastResult.deletedOld > 0 ? `, xóa ${lastResult.deletedOld} cũ` : ''}
                                </span>
                            )}
                        </div>
                    </div>
            ...
            <div className="admin-page-pad" style={{ padding: '28px 36px' }}>
                ...
                >
                    <div style={{ color: '#7C6D9B', fontSize: 14, lineHeight: 1.7 }}>
                        Chức năng sao lưu dữ liệu đang được chuẩn bị cho môi trường vận hành chính thức.
                    </div>
                </div>
            </div>
        )
            ...
            >
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    <button
                        ...
                        aria-label="Làm mới danh sách backup"
                    >
                        Làm mới
                    </button>
                    <button
                            ...
                            <tr key={backup.name} style={{ borderBottom: '1px solid #EDE9FE' }}>
                                <td style={{ padding: '12px 16px' }}>
                                    ...
                                    <div style={{ fontSize: 11, color: '#9B93C9' }}>
                                        ...
                                        {backup.actor?.name ? ` · ${backup.actor.name}` : ''}
                                    </div>
                                </td>
                                <td style={{ padding: '12px 16px', fontSize: 13, color: '#6B6494' }}>
                                    {fmtDate(backup.createdAt || backup.modifiedAt)}
                                </td>
                                <td style={{ padding: '12px 16px', fontSize: 13, color: '#6B6494' }}>
                                    {fmtSize(backup.size)}
                                </td>
                                ...
                                <td style={{ padding: '12px 16px' }}>
                                    <div style={{ display: 'flex', gap: 8 }}>
                                        <button
                                            ...
                                            aria-label={`Tải backup ${backup.name}`}
                                        >
                                            Tải
                                        </button>
                                        <button
                                            ...
                                            aria-label={`Khôi phục backup ${backup.name}`}
                                        >
                                            Khôi phục
                                        </button>
                                    </div>
...
function SupabaseStorageMaintenance() {
    ...
    async function removeArchived() {
        if (!confirm('Xóa toàn bộ ảnh đang ở trạng thái lưu trữ khỏi hệ thống? Hãy tải ảnh xuống máy trước khi xóa.'))
            return
        setBusy(true)
        setMessage('')
        setError('')
    ...
    return (
        <div className="admin-page-pad" style={{ padding: '28px 36px' }}>
            <div
                className="mobile-stack"
                style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 12, marginBottom: 20 }}
            >
                <button
                    onClick={loadSummary}
            ...
            >
                ...
                >
                    <span>
                        {percent}% so với ngưỡng cảnh báo {fmtSize(summary?.warningBytes || 0)}
                    </span>
                ...
                {summary?.isWarning && (
                    ...
                    >
                        Dung lượng ảnh đã vượt ngưỡng cảnh báo. Hãy tải ảnh lưu trữ xuống máy rồi xóa khỏi hệ thống nếu
                        không còn cần hiển thị trên cổng phụ huynh.
                    </div>
                )}
            </section>
            ...
            >
                ...
                <div style={{ color: '#7C6D9B', fontSize: 13, lineHeight: 1.7, marginBottom: 16 }}>
                    Ảnh cần xóa nên được chuyển sang trạng thái lưu trữ trong Thư viện ảnh. Tải về máy trước, sau đó mới
                    xóa khỏi hệ thống.
                </div>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    <button
                        onClick={downloadArchived}
                        disabled={busy || loading || !summary?.archivedCount}
                        style={{
                            padding: '10px 18px',
                            borderRadius: 10,
                            border: '1.5px solid #7C3AED',
