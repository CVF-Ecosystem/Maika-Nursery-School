...
export default function ParentLogin() {
    ...
    async function handleLogin(e) {
        ...
        if (!legacyLoginAllowed || !DEMO_MODE) {
            setErr('Vui lòng đăng nhập bằng email và mật khẩu trên Cổng phụ huynh.')
            setLoading(false)
            return
        }
    ...
    return (
        <div
            style={{
                minHeight: '100vh',
                background: 'linear-gradient(135deg,#1E1B4B,#2D2870,#4C1D95)',
                display: 'flex',
                alignItems: 'center',
                ...
                <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {useSupabaseLogin ? (
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder={DEMO_MODE ? '0901234567' : 'Nhập số điện thoại'}
                                autoFocus
                                style={{
                                    width: '100%',
                                    padding: '12px 16px',
                    ...
                    <button
                        ...
                        style={{
                            ...
                            borderRadius: 12,
                            border: 'none',
                            background: loading ? '#DDD6FE' : 'linear-gradient(135deg,#6D28D9,#8B5CF6)',
                            color: loading ? '#7C6D9B' : '#fff',
                            fontWeight: 800,
                            fontSize: 15,
                            boxShadow: loading ? 'none' : '0 6px 20px rgba(109,40,217,0.4)',
                            cursor: loading ? 'not-allowed' : 'pointer',
                        }}
                    >
                        {loading ? 'Đang đăng nhập...' : 'Đăng nhập →'}
                    </button>
                    {legacyLoginAllowed && !useSupabaseLogin && DEMO_MODE && (
                        <div
                            style={{
                                fontSize: 12,
                                color: '#9B93C9',
                        ...
                        >
                            Demo: <strong style={{ color: '#6D28D9' }}>0901234567</strong>
                        </div>
                    )}
                    <button
                        type="button"
