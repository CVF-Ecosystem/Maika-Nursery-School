// bb-sidebar.jsx — Navigation sidebar + App shell (Dark Indigo theme)
const NAV_ITEMS = [
...
function Sidebar({ active, onNav, unreadCount }) {
  return (
    <aside style={{
      ...
      <div style={{ padding: '22px 20px 18px', borderBottom: '1px solid rgba(167,139,250,0.15)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          <div style={{
            ...
            fontSize:22, boxShadow:'0 4px 14px rgba(124,58,237,0.5)'
          }}>🌸</div>
          <div>
            <div style={{ fontWeight:900, fontSize:17, color:'#FFFFFF', letterSpacing:-0.5 }}>Maika</div>
            ...
            {NAV_ITEMS.filter(item => group.ids.includes(item.id)).map(item => {
              ...
              return (
                <button key={item.id} onClick={() => onNav(item.id)} style={{
                  ...
                  fontFamily:'Nunito, sans-serif', fontSize:13.5, fontWeight: isActive ? 700 : 600,
                ...
                onMouseLeave={e => { if (!isActive) { e.currentTarget.style.background='transparent'; e.currentTarget.style.color='#8B83C3'; }}}>
                  ...
                  {item.id === 'messages' && unreadCount > 0 && (
                    <span style={{
                      ...
                      fontSize:10, fontWeight:900, padding:'2px 7px', minWidth:20, textAlign:'center',
      ...
      {/* User info */}
      <div style={{
        padding:'14px 16px', borderTop:'1px solid rgba(167,139,250,0.15)',
        ...
        <div style={{ width:8, height:8, borderRadius:999, background:'#10B981', boxShadow:'0 0 0 2px rgba(16,185,129,0.3)' }}></div>
      </div>
    </aside>
  );
}
...
function TopBar({ title, subtitle }) {
  ...
  return (
    <div style={{
      height:64, background:'#fff', borderBottom:'1px solid #EDE9FE',
