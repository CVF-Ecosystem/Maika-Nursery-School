-- Fix get_public_landing to count teachers from public.teachers (roster),
-- not public.profiles (login accounts). profiles only counts staff with accounts.
...
begin
  ...
  -- Count from teachers roster, not login accounts
  begin
