// bb-data.js — Maika Data Layer (localStorage persistence)
...
function getDB() {
  if (!window._bbDB) window._bbDB = loadData();
  return window._bbDB;
}

function commit() {
  saveData(window._bbDB);
}
...
window.BB = { getDB, commit, fmtDate, fmtMoney, fmtDateTime, getAge, todayStr, defaultData };
